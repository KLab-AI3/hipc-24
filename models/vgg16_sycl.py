...
# ConvLayer with dynamic algo selection
...
# VGG16 model with SYCL-enabled ConvLayers
class VGG16_Sycl(nn.Module):
    def __init__(self, in_channels=3, num_classes=1000, algo="smm"):
        super().__init__()
        self.conv1 = self.make_block(in_channels, [64, 64], algo)
        self.conv2 = self.make_block(64, [128, 128], algo)
        self.conv3 = self.make_block(128, [256, 256, 256], algo)
        self.conv4 = self.make_block(256, [512, 512, 512], algo)
        self.conv5 = self.make_block(512, [512, 512, 512], algo)
    ...
    def make_block(self, in_channels, channels, algo):
        layers = []
        for out_channels in channels:
            layers.append(ConvLayer(in_channels, out_channels, kernel_size=(3,3), stride=1, padding=1, algo=algo))
...
if __name__ == "__main__":
    ...
    model = VGG16_Sycl(algo="smm").to(device)
